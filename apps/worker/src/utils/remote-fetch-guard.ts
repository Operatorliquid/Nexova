...
export async function fetchBinaryWithGuards(params: {
  ...
  const maxBytes = Number.isFinite(params.maxBytes) ? Math.max(1, Math.trunc(params.maxBytes)) : 25 * 1024 * 1024;
  ...
  try {
    ...
    if (payload.byteLength > maxBytes) {
      throw new Error(`Audio file too large (${payload.byteLength} bytes)`);
    }
    ...
    if (msg.toLowerCase().includes('abort')) {
      throw new Error('Timeout al descargar media remota');
    }
    throw error;
  } finally {
